...
namespace Reluare_priectre
{
    public class Game1 : Game
    {
        ...
        static public int[] OBTIUNI = { 0, 768, 1366, 0, 1, 100, 100, 1600, 0, 1 };
        ...
        static public int[] keys_pressed = new int[45];
        ...
        protected override void Update(GameTime gameTime)
        {
           /* if (GamePad.GetState(PlayerIndex.One).Buttons.Back == ButtonState.Pressed)
            ...
            if (pl_scrie_CHAT > 0)
            {
                COMANDA.WRITE_CHAT_LINE(); Keyboard.GetState();
                if (Keyboard.GetState().IsKeyDown(Keys.Enter))
                {
                    COMANDA.comanda(CHAT[0]);
            ...
            {
                pl_scrie_CHAT = 1;
                keys_pressed['Z' - 'A' + 14] = 1;
                COMANDA.ADD_CHAT_LINE(">");
            }
            ...
            else if (MENU == 3) // MENIUL DE JOC SPATIU
            {
                ...
                for (int i = 0; i < NR_PRO - 1; i++)
                    if (LAS[i].tip_p == 1)
                    {
                        ...
                        {
                            dist = MATH.dis(NPC[j].poz, LAS[i].poz);
                            if (dist <= 20 * 30)
                            {
                    ...
                    {
                        dist = MATH.dis(PL.poz, LAS[i].poz);
                        if (dist <= 20 * 30)
                        {
            ...
            else if (MENU == 4)  // MENIUL DE CONSTRUCTIE A NAVEI
            {
                ...
                if (Mouse.GetState().LeftButton == ButtonState.Pressed)  /// PUNERE/SCOATERE COMPONENTE
                {
                    ...
                    else
                    {
                        int i, j;
                        float au_i, au_j;
                        au_i = PL_P_E.X - MOUSE_P.X;
                        au_j = PL_P_E.Y - MOUSE_P.Y;
            ...
            else if (MENU == 5)  // PE PLANETA
            {
                ...
                if (L_PLA[PLA_A].ord_elm[NR_subs + 1] != 0 && L_PLA[PLA_A].ord_elm[NR_subs + 1] != NR_subs + 4)
                {
                    if (Mouse.GetState().LeftButton == ButtonState.Pressed)
                    {
                        if (PL_P.parti[0] == 1)
                            ...
                            if (inventar[PL_P.parti[1]] >= 0)
                            {
                                ...
                                if (Math.Sqrt(x * x + y * y) <= 80 && Math.Sqrt(x * x + y * y) >= 30)
                                {
                                    ...
                                    if (x > 0 && x < 300 && y > 0 && y < 300)
                                        if (PLA_S.a[x, y] == 0)
                                        {
                                            if (PL_P.parti[1] > NR_comp && PL_P.parti[1] <= NR_comp + NR_elem * NR_subs)
                                            {
            ...
            else if (MENU == 11)  // PE NAVA
            {
               ...
               /* PL_P.poz += new Vector2((float)Math.Sin(3.1415926f / 2 - MATH.ung(PL_P_E, MOUSE_P)) * PL_P.fy,
                ...
                for (int d = 0; d < 4; d++)
                {
                    PL_P.X = (int)((PL_P.poz.Y + 10 + 7 * d1[d]) / 20);
        ...
        protected override void Draw(GameTime gameTime)
        {
            ...
            if (MENU == 1)
            {
                ...
                for (int i = 1; i <= NR_PRO; i++)
                {
                    if (i <= NR_PRO - NR_PRO % 20)
                        ...
                        spriteBatch.Draw(PLA_TEX[7], LAS[i].poz, null, Color.White, (float)LAS[i].pow / 50, new Vector2(PLA_TEX[7].Height / 2, PLA_TEX[7].Width / 2), 0.08f, SpriteEffects.None, 0f);
                }
            ...
            else if (MENU == 5)
            {
                ...
                #region AFISARE_FRONT_LAYER
                MM = (int)((WINDOW_REZ.X / 40) / ZOOM) + 2;
                NN = (int)((WINDOW_REZ.Y / 40) / ZOOM) + 2;
                for (int ii = (int)(-MM * 1); ii <= (int)(MM * 1); ii++)
                    if (y + ii > 0 && y + ii < 300)
                    {
                        int i = y + ii;
                        ...
                        if (x > 0 && x < 300 && y > 0 && y < 300)
                        {
                            if (PLA_S.b[x, y] != 0)
                            {
                                ...
                                if (PLA_S.b[x, y] <= 0)
                                {
                                    ...
                                    PLA_S.b[x, y] = 0;
                                    ELIMINARE_APA(x - 1, y);
                                    if (PLA_S.a[x, y] / 100 == 3 || PLA_S.a[x, y] / 100 == 4 || PLA_S.a[x, y] / 100 == 13)
                                        PLA_S.a[x, y] = 0;
